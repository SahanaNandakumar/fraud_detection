import joblib
import pandas as pd
import numpy as np

model = joblib.load('fraud_model.pkl')
scaler = joblib.load('scaler.pkl')

fraud = [406, -2.3122265423263, 1.95199201064158, -1.60985073229769,
3.9979055875468, -0.522187864667764, -1.42654531920595, -2.53738730624579,
1.39165724829804, -2.77008927719433, -2.77227214465915, 3.20203320709635,
-2.89990738849473, -0.595221881324605, -4.28925378244217, 0.389724120274487,
-1.14074717980657, -2.83005567450437, -0.0168224681808257, 0.416955705037907,
0.126910559061474, 0.517232370861764, -0.0350493686052974, -0.465211076182388,
0.320198198514526, 0.0445191674733725, 0.177839798284401, 0.261145002567677,
-0.143275874698919, 149.62]

cols = ['Time','V1','V2','V3','V4','V5','V6','V7','V8','V9',
'V10','V11','V12','V13','V14','V15','V16','V17','V18',
'V19','V20','V21','V22','V23','V24','V25','V26','V27','V28','Amount']

df = pd.DataFrame([fraud], columns=cols)
print("Before scaling:", df[['Amount','Time']].values)

df[['Amount','Time']] = scaler.transform(df[['Amount','Time']])
print("After scaling:", df[['Amount','Time']].values)

prob = model.predict_proba(df.values)[0][1]
print('Fraud probability:', prob)
